...
const API = {
    ...
    async settings() { return (await this.request("/api/settings")).settings; },
    async updateSettings(data) { return (await this.request("/api/settings",{method:"PUT",body:JSON.stringify(data)})).settings; },
    async chats() { return this.request("/api/chats"); },
    async groups() { return (await this.request("/api/groups")).groups; },
    async createGroup(data) { return (await this.request("/api/groups",{method:"POST",body:JSON.stringify(data)})).group; },
    async groupMessages(id) { return (await this.request(`/api/groups/${id}`)).messages; },
    async sendGroup(id,text,replyTo=null) { return (await this.request(`/api/groups/${id}`,{method:"POST",body:JSON.stringify({text,replyTo})})).message; },
    async sendGroupAudio(id,audio,replyTo=null) { return (await this.request(`/api/groups/${id}`,{method:"POST",body:JSON.stringify({audio,replyTo})})).message; },
    async sendGroupMedia(id,media,replyTo=null) { return (await this.request(`/api/groups/${id}`,{method:"POST",body:JSON.stringify({media,replyTo})})).message; },
